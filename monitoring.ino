...
void kirimFirebase() {
    ...
    if (Firebase.RTDB.setJSON(&fbdo, "/devices/esp32_01", &json)) {
...
void setup() {
    ...
    pinMode(PIN_POT_VOLT, INPUT);
    pinMode(PIN_POT_ARUS, INPUT);

    // OLED
    if (!display.begin(OLED_ADDR, true)) {
        Serial.println("SH110X tidak terdeteksi!");
        while (true) {
            delay(1000);
        }
    }
    ...
    display.clearDisplay();
    display.setCursor(0, 0);
    display.println("WiFi: JIK-2024");

    display.setCursor(0, 12);
    display.println(WiFi.status() == WL_CONNECTED ? "Status: Terhubung" : "Status: Gagal");
...
// ============================================================
void bacaSensor() {
    int rawVolt = analogRead(PIN_POT_VOLT);
    int rawArus = analogRead(PIN_POT_ARUS);

    // Tegangan: 1 - 500 V
    tegangan = 1.0 + ((float)rawVolt / 4095.0) * (500.0 - 1.0);

    // Arus: 0 - 10 A
    arus = ((float)rawArus / 4095.0) * 10.0;
...
void tampilkanOLED() {
    ...
    display.setTextSize(1);
    display.setCursor(0, 0);
    display.println("MONITOR DAYA LISTRIK");
    ...
    display.print(" I:");
    display.print(arus, 2);

    display.setCursor(0, 26);
    display.print("Daya: ");
    display.print(daya, 1);
    display.println(" W");
